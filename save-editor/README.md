# Shared XP Pool -- save editor

Converges a world's players to one XP total, once, offline.

**Most people want [the mod](../mod/) instead**, which does this continuously
while people play. This is for worlds the mod has never run on, for putting one
right by hand, or for a group that wants the average rather than the top.

Works end-to-end on a real save.
...
- **Offline-only.** This half is a save editor. The live mod is in [`../mod/`](../mod/).
...
The mod's own suite is separate: `lua mod/tests/test_pool.lua` from the repo
root.
